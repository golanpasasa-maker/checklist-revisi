/**
 * ui.js
 * Menangani semua operasi DOM:
 * render checklist, update progress bar,
 * animasi confetti, collapse/expand kategori.
 */
...
function getTotals(state) {
  let total = 0, done = 0;
  state.forEach(cat =>
    cat.items.forEach(item => { total++; if (item.done) done++; })
  );
  return { total, done };
}
...
function updateProgress(state) {
  ...
  document.getElementById("countDisplay").innerHTML = `${done} <span>/ ${total} selesai</span>`;
 ...
 * @param {Function} onToggle - Callback(catIdx, itemIdx, itemEl)
 */
function renderChecklist(state, onToggle) {
  ...
  state.forEach((cat, catIdx) => {
    ...
    const pillClass = doneCat === totalCat ? "all-done"
...
function showSyncStatus(isOnline) {
  ...
  indicator.innerHTML = isOnline
    ? `<span style="color:#22d37a">⬤</span> <span style="color:#6b7280">Sync aktif — tersimpan di semua perangkat</span>`
    : `<span style="color:#f7c94f">⬤</span> <span style="color:#6b7280">Mode offline — data tersimpan di perangkat ini saja</span>`;
}
